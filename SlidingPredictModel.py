...
def sliding_predict_model(initial_fund, test_set, train_set, predict_length = 15, step = 0.75, price_indexes ="average", cmd=(0, 1)):
    fund = initial_fund
    had = 0
    ...
    for i in range(len(price_list)):
        ...
        for j in range(1, predict_length - 1):
            seq.append(price_list[i+j] / seq[0])
        # save the seq and the price at last
        train_data.append((seq, price_list[i + predict_length - 1] - seq[0]))
